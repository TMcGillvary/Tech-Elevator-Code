...
function iqTest(numberString) {
    ...
    if (evenCount < oddCount) {
        for (let i = 0; i < numArray.length; i++) {
            if (numArray[i] % 2 === 0) {
                position = numArray.indexOf(numArray[i]) + 1;
                break;
            }
        }
...
function titleCase(stringOG, minorWords = "") {
        ...
        if (optionalWordsArray.length > 0) {
            ...
            for (let j = 0; j < optionalWordsArray.length; j++) {
                if (stringArray[i] === stringArray[0]) {
                    ...
                    break;
                } else if (stringArray[i].toLowerCase() === optionalWordsArray[j]) {
    ...
    let fixedCase = stringArray.join(" ");
