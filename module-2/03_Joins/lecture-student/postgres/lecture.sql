...
SELECT park_name, state_name, city_name AS capital_city, park.area
FROM park 
JOIN park_state ON park.park_id = park_state.park_id 
JOIN state ON park_state.state_abbreviation = state.state_abbreviation
JOIN city ON state.capital = city.city_id;

-- you can give your table names aliases (essentially variable names) so you don't have to type out the name of each table so many times
SELECT park_name, state_name, city_name AS capital_city, p.area
FROM park AS p
...
SELECT state_name, COUNT(*) AS num_of_cities
FROM city
JOIN state ON city.state_abbreviation = state.state_abbreviation
WHERE census_region = 'Midwest'
...
SELECT 
  state_name, MIN(date_established) AS earliest_established_park
FROM 
  state
...
GROUP BY 
  state_name
ORDER BY 
  earliest_established_park;

-- Modify the previous query so the results include entries for all the records in the state table, even if they have no park records associated with them.
SELECT state_name, MIN(date_established) AS earliest_established_park
FROM state
