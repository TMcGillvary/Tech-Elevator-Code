...
SELECT 'Hello, world!';
-- select 'Hello, world!'; -- is also possible
-- Select 'Hello, world!'; -- is also possible
...
SELECT * from park; -- shorthand for all columns is * wildcard
...
-- the data within the table is case sensitive, always single quotes string
...
-- even though number is a numeric postgres can recognize 25 vs 25.0 as the same
...
-- have to do keyword 'IS' not '=' for null values
...
-- the not equals or != equivalent is IS NOT
...
SELECT city_name, state_abbreviation, area, population FROM city 
  WHERE area < 25.0 AND population > 100000;
...
SELECT park_name, date_established FROM park WHERE date_established < '1900-01-01'; -- date literal syntax
