...
FROM 
        park 
ORDER BY 
        date_established ASC, -- or age_in_years DESC, is more appropriate because we're not sorting by date_established anymore
        park_name ASC 
LIMIT 20;
...
-- || is the concat operator for SQL
-- can still sort OG columns like normal even with concat
-- you can do this but it's not recommended because you're changing the info in the database and passing that up 
-- rather than passing the raw data and having the front end display it the correct way
...
SELECT COUNT(*) AS city_pop_over_1mill FROM city WHERE population > 1000000;

-- The number of state nicknames.
SELECT COUNT(state_nickname) FROM state; -- primary key search rather than *
-- also possible to do
...
SELECT 
        state_abbreviation, 
        MIN(population) AS smallest_city_pop 
FROM 
        city 
GROUP BY 
        state_abbreviation 
ORDER BY 
        smallest_city_pop ASC;
...
SELECT city_name, state_abbreviation FROM city ORDER BY state_abbreviation, city_name LIMIT 10 OFFSET 10;
